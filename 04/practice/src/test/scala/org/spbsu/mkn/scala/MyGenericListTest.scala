...
class MyGenericListTest extends AnyFunSuite {
  ...
  test("map") {
    assert(MyNil.map((i: Int) => i * 2) == MyNil)
    assert(fromSeq(Seq(1, 2, 3)).map((i: Int) => i * 2) == fromSeq(Seq(2, 4, 6)))
    assert(fromSeq(Seq(1, 2, 3)).map(identity[Int]) == fromSeq(Seq(1, 2, 3)))
  }
  ...
  test("sort") {
    assert(sort(fromSeq(Seq(4, 3, 2, 1))) == fromSeq(Seq(1, 2, 3, 4)))
    assert(sort(fromSeq(Seq(2 -> 3, 2 -> 1))) == fromSeq(Seq(2 -> 1, 2 -> 3)))
    assert(sort(fromSeq(Seq(2 -> 3, 2 -> 1, 1 -> 5))) == fromSeq(Seq(1 -> 5, 2 -> 1, 2 -> 3)))
    assert(sort(fromSeq(Seq(3 -> 2, 2 -> 1, 1 -> 5))) == fromSeq(Seq(1 -> 5, 2 -> 1, 3 -> 2)))
  }
